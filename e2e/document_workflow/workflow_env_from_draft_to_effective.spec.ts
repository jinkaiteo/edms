...
// Author creates draft and submits for review
test('author creates draft document and submits for review', async ({ page }) => {
...
test('reviewer approves document', async ({ page }) => {
...
// Author routes for approval
test('author routes for approval', async ({ page }) => {
  await login(page, AUTHOR_USERNAME, AUTHOR_PASSWORD);
  await page.getByRole('link', { name: 'My Tasks' }).click();
  await expect(page.getByRole('main')).toContainText('REVIEWED');
  await page.getByRole('button', { name: 'workflow' }).click();
...
// Approver approves and document becomes effective
test('approver final approval and effective', async ({ page }) => {
