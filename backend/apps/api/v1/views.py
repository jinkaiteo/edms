...
class AuditTrailViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def export_pdf(self, request):
        ...
        # Title
        title = Paragraph("Audit Trail Report", title_style)
        elements.append(title)
        
        # Metadata
        metadata_style = styles['Normal']
        metadata = Paragraph(
            f"<b>Generated:</b> {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}<br/>"
            f"<b>Total Entries:</b> {queryset.count()}<br/>"
        ...
        # Table data
        data = [['Timestamp', 'User', 'Action', 'Description']]
        
        for entry in queryset[:100]:  # Limit to first 100 entries for PDF
            data.append([
                entry.timestamp.strftime('%Y-%m-%d\n%H:%M:%S') if entry.timestamp else '',
                entry.user.username if entry.user else 'System',
        ...
        if queryset.count() > 100:
            elements.append(Spacer(1, 20))
            note = Paragraph(
                f"<i>Note: Showing first 100 of {queryset.count()} entries. "
                "Use filters or CSV export for complete data.</i>",
                styles['Normal']
            )
        ...
        response['Content-Disposition'] = f'attachment; filename="audit_trail_{timezone.now().strftime("%Y%m%d_%H%M%S")}.pdf"'
