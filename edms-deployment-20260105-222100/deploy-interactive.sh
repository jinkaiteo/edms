...
setup_backup_system() {
    ...
    echo "The EDMS backup system provides automated database and storage backups."
    echo ""
    
    if prompt_yes_no "Would you like to configure automated backups now?" "y"; then
        print_step "Configuring backup system..."
        
        # Check if backup scripts exist
        if [ ! -f "scripts/backup-edms.sh" ]; then
            print_warning "Backup scripts not found in scripts/ directory"
            print_info "You can configure backups later by running: ./scripts/setup-backup-cron.sh"
            return 0
        fi
        
        # Make scripts executable
        chmod +x scripts/backup-edms.sh 2>/dev/null || true
        chmod +x scripts/restore-edms.sh 2>/dev/null || true
        chmod +x scripts/setup-backup-cron.sh 2>/dev/null || true
        chmod +x scripts/verify-backup.sh 2>/dev/null || true
        
        print_info "Available backup scripts:"
        echo "  - backup-edms.sh         Create backups"
        echo "  - restore-edms.sh        Restore from backup"
        echo "  - setup-backup-cron.sh   Configure automated backups"
        echo "  - verify-backup.sh       Verify backup integrity"
        echo ""
        
        if prompt_yes_no "Run the backup configuration script now?" "y"; then
            print_step "Launching backup configuration..."
            echo ""
            
            # Set environment variables for backup script
            export POSTGRES_CONTAINER="${DB_NAME}_db"
            export POSTGRES_USER="${DB_USER}"
            export POSTGRES_DB="${DB_NAME}"
            
            # Run the setup script
            if ./scripts/setup-backup-cron.sh; then
                print_success "Backup system configured successfully!"
            else
                print_warning "Backup configuration was cancelled or failed"
                print_info "You can run it later: ./scripts/setup-backup-cron.sh"
            fi
        else
            print_info "Backup configuration skipped"
            echo ""
            print_info "To configure backups later, run:"
            echo "  cd $(pwd)"
            echo "  export POSTGRES_CONTAINER='${DB_NAME}_db'"
        ...
        print_info "Backup configuration skipped"
        print_info "You can configure backups anytime by running: ./scripts/setup-backup-cron.sh"
    fi
...
show_final_summary() {
    ...
    echo "  4. Configure backup automation (run: ./scripts/setup-backup-cron.sh)"
    if [ "$USE_HAPROXY" = true ]; then
    ...
    echo "  - METHOD2_BACKUP_RESTORE_REFERENCE.md - Backup & restore guide"
    if [ "$USE_HAPROXY" = true ]; then
