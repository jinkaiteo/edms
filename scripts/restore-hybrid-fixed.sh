#!/bin/bash
#
# EDMS Hybrid Restore - Production Ready (Auto-detect compose file)
...
# Smart compose file detection
if [ -n "$COMPOSE_FILE" ]; then
    # Use explicitly set COMPOSE_FILE from .env
    COMPOSE_FILE="$COMPOSE_FILE"
elif docker compose -f docker-compose.prod.yml ps >/dev/null 2>&1; then
    # Production file has running containers
    COMPOSE_FILE="docker-compose.prod.yml"
    echo "✅ Auto-detected: docker-compose.prod.yml"
elif docker compose -f docker-compose.yml ps >/dev/null 2>&1; then
    # Development file has running containers
    COMPOSE_FILE="docker-compose.yml"
    echo "✅ Auto-detected: docker-compose.yml"
else
    # Fallback to production (safer default)
    COMPOSE_FILE="docker-compose.prod.yml"
    echo "⚠️  No running containers detected, using: docker-compose.prod.yml"
fi
